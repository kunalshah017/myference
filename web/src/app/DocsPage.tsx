...
ollama list`}</CopyBlock></Step>
            <Step number="03" title="Connect and discover"><p>Run the guided host command. It discovers the installed model, records its Ollama digest, starts browser login when needed, opens the provider workspace, and serves in the foreground.</p><CopyBlock label="Terminal or PowerShell">{`myference host --model qwen2.5:0.5b`}</CopyBlock><p className="docs-note">Use <code>.\myference.exe</code> on Windows if its folder is not on PATH. Add <code>--no-browser</code> to print URLs instead.</p></Step>
