...
function LandingPage() {
  return <div className="landing-page">
    ...
    <main>
      ...
      <section id="how-it-works" className="landing-process"><div className="section-heading"><div><p className="eyebrow">How it works</p><h2>Prompt to settlement.</h2></div><span className="data-label">Live only</span></div><div className="landing-process-grid">{steps.map(({ icon: Icon, label, body }, index) => <article key={label}><span>{String(index + 1).padStart(2, '0')}</span><Icon size={23} /><h3>{label}</h3><p>{body}</p></article>)}</div><a className="landing-secondary landing-docs-link" href="/docs">See full documentation <ArrowRight size={17} /></a></section>
