...
export type MarketOffer = { machine_id: string; provider_address: `0x${string}`; offer_id: string; model: string; capabilities: string[]; price_version: number; input_per_million_wei: string; output_per_million_wei: string; compute_per_second_wei: string; capacity: number; latency_milliseconds: number; success_basis_points: number; reputation: number; evidence_kind: 'provider_claimed' | 'ollama_digest' | 'upstream_model' | 'runtime_image'; evidence_digest: string; metering_mode: 'tokens_and_compute' | 'compute_only'; available: boolean; stale: boolean; updated_at: string }
