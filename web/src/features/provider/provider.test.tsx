...
it('shows real machines, immutable offer versions, collateral, and earnings', async () => {
  ...
  await userEvent.type(screen.getByLabelText(/offer name/i),'local-qwen');await userEvent.type(screen.getByLabelText(/^model$/i),'qwen');await userEvent.click(screen.getByLabelText(/temporary workspace/i));await userEvent.type(screen.getByLabelText(/input tokens/i),'10');await userEvent.type(screen.getByLabelText(/output tokens/i),'20');await userEvent.type(screen.getByLabelText(/compute time/i),'30');await userEvent.click(screen.getByRole('button',{name:/publish next offer/i}))
