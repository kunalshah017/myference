...
export type ProviderActionPhase = 'wallet' | 'chain' | 'refresh'
...
export async function executeProviderAction(action: ProviderAction, writer: MarketWriter, api: ProviderAPI, wait: Wait = (milliseconds) => new Promise((resolve) => setTimeout(resolve, milliseconds)), onPhase?: (phase: ProviderActionPhase) => void) {
