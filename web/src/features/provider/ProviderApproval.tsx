...
import { ProviderAPI, type ProviderAction } from '../../lib/api'
...
export function ProviderApproval({ actionID, api = new ProviderAPI(), writer: supplied, checkWallet = assertProviderWallet }: { actionID: string; api?: ProviderAPI; writer?: MarketWriter; checkWallet?: (action: ProviderAction) => Promise<void> }) {
  ...
  if (account.isError || action.isError) return <main className="approval-page"><p role="alert">This provider action is unavailable, expired, or belongs to another account.</p></main>
  const approve = async () => {
