...
export function ModelList({ api = new MarketplaceAPI() }: { api?: MarketplaceAPI }) {
  ...
  return <div className="model-index">{models.data.map((model) => <article key={model.model} className="model-line" data-stale={model.stale}><button type="button" className="model-select" onClick={() => setSelected(selected === model.model ? '' : model.model)} aria-expanded={selected === model.model}><strong>{model.model}</strong><span>{model.stale ? 'Capacity is stale' : `${model.available_providers} provider · ${model.total_capacity} slots`}</span><span>from {model.minimum_input_per_million_wei} wei / 1M input</span></button>{selected === model.model && <ModelDetail api={api} model={model.model} />}</article>)}</div>
}
