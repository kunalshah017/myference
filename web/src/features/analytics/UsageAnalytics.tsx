...
export function UsageAnalytics({ api = new AnalyticsAPI() }: { api?: AnalyticsAPI }) {
  ...
  const metrics = [{ label: 'Settled requests', value: customer.settled_requests, icon: Activity }, { label: 'Input tokens', value: customer.input_tokens, icon: Hash }, { label: 'Output tokens', value: customer.output_tokens, icon: Hash }, { label: 'Compute time', value: `${customer.compute_milliseconds} ms`, icon: Clock3 }, { label: 'Provider charges', value: `${customer.provider_charges_wei} wei`, icon: Coins }, { label: 'Protocol fees', value: `${customer.protocol_fees_wei} wei`, icon: Coins }, { label: 'Total spent', value: `${customer.total_spent_wei} wei`, icon: Coins }]
  return <div className="analytics-panel"><div className="analytics-metrics">{metrics.map(({ label, value, icon: Icon }) => <article key={label}><Icon size={18} aria-hidden="true" /><span>{label}</span><strong>{value}</strong></article>)}</div><section className="analytics-section"><div className="section-heading"><div><p className="eyebrow">Last 30 days</p><h2>Confirmed requests</h2></div></div><DailyChart days={daily} field="customer_requests" /></section><section className="analytics-section"><p className="eyebrow">Settlement ledger</p><h2>Recent usage</h2>{usage.length === 0 ? <p className="activity-empty">No confirmed inference usage yet.</p> : <div className="analytics-table"><div className="analytics-row head"><span>Request / model</span><span>Tokens</span><span>Compute</span><span>Total</span><span>Confirmed</span></div>{usage.map((item) => <div className="analytics-row" key={item.request_id}><div><code>{item.request_id}</code><small>{item.model}</small></div><span>{item.input_tokens} in / {item.output_tokens} out</span><span>{item.compute_milliseconds} ms</span><strong>{item.total_charge_wei} wei</strong><time dateTime={item.completed_at}>{new Date(item.completed_at).toLocaleString()}</time></div>)}</div>}</section></div>
}
