...
export function ApiKeys({ api = new AuthAPI() }: { api?: AuthAPI }) {
  ...
  return <section className="auth-section" aria-labelledby="api-keys-title"><p className="eyebrow">Project access</p><h2 id="api-keys-title">Scoped API keys</h2>{revealed && <div className="secret-proof" role="status"><span>Copy now — this key is shown once</span><code>{revealed}</code><button type="button" onClick={() => setRevealed('')}>I saved this key</button></div>}<form className="key-form" onSubmit={create}><label htmlFor="key-model">Model</label><input id="key-model" value={model} onChange={(event) => setModel(event.target.value)} required /><label htmlFor="key-maximum">Maximum spend (wei)</label><input id="key-maximum" inputMode="numeric" value={maximum} onChange={(event) => setMaximum(event.target.value)} required /><button type="submit">Create API key</button></form>{error && <p role="alert" className="inline-error">{error}</p>}<ul className="key-list">{keys.map((key) => <li key={key.id}><div><code>{key.id}</code><span>{key.scope.models.join(', ')}</span><span>{key.scope.endpoints.join(', ')}</span><span>{String(key.scope.max_spend_wei)} wei max</span></div><button type="button" onClick={() => void revoke(key.id)}>Revoke {key.id}</button></li>)}</ul></section>
}
